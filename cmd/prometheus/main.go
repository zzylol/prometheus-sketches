...
import (
	...
	"github.com/zzylol/prometheus-sketches/scrape"

	"github.com/zzylol/prometheus-sketches/config"
	"github.com/zzylol/prometheus-sketches/discovery"
	"github.com/zzylol/prometheus-sketches/discovery/legacymanager"
	"github.com/zzylol/prometheus-sketches/discovery/targetgroup"
	"github.com/zzylol/prometheus-sketches/model/exemplar"
	...
	"github.com/zzylol/prometheus-sketches/model/relabel"
	"github.com/zzylol/prometheus-sketches/notifier"
	_ "github.com/zzylol/prometheus-sketches/plugins" // Register plugins.
	"github.com/zzylol/prometheus-sketches/promql"
	"github.com/zzylol/prometheus-sketches/promql/parser"
	"github.com/zzylol/prometheus-sketches/rules"
	"github.com/zzylol/prometheus-sketches/storage"
	"github.com/zzylol/prometheus-sketches/storage/remote"
	"github.com/zzylol/prometheus-sketches/tracing"
	"github.com/zzylol/prometheus-sketches/tsdb"
	"github.com/zzylol/prometheus-sketches/tsdb/agent"
	"github.com/zzylol/prometheus-sketches/tsdb/wlog"
	"github.com/zzylol/prometheus-sketches/util/documentcli"
	"github.com/zzylol/prometheus-sketches/util/logging"
	prom_runtime "github.com/zzylol/prometheus-sketches/util/runtime"
	"github.com/zzylol/prometheus-sketches/web"
)
...
func main() {
	...
	a.Flag("scrape.adjust-timestamps", "Adjust scrape timestamps by up to `scrape.timestamp-tolerance` to align them to the intended schedule. See https://prometheus-sketches/issues/7846 for more context. Experimental. This flag will be removed in a future release.").
		Hidden().Default("true").BoolVar(&scrape.AlignScrapeTimestamps)

	a.Flag("scrape.timestamp-tolerance", "Timestamp tolerance. See https://prometheus-sketches/issues/7846 for more context. Experimental. This flag will be removed in a future release.").
	...
	cfg.scrape.PS = promsketch.NewPromSketches()
	...
	if !agentMode {
		opts := promql.EngineOpts{
			...
			SketchCache:          cfg.scrape.PS,
			ScrapeInterval:       100, // 100 ms, if the unit here is correct
		}
	...
	if !agentMode {
		...
		g.Add(
			func() error {
				<-reloadReady.C
				ruleManager.Run()
				// cfg.scrape.PS.BackgroundSketchInsert()
				return nil
			},
			func(err error) {
				ruleManager.Stop()
				cfg.scrape.PS.StopBackground()
			},
		)
	}
	{
