...
import (
	...
	"github.com/zzylol/prometheus-sketches/util/httputil"
...
func (api *API) query(r *http.Request) (result apiFuncResult) {
	...
	qry, err := api.QueryEngine.NewInstantQuery(ctx, api.Queryable, opts, r.FormValue("query"), ts, false)
	if err != nil {
...
func (api *API) queryRange(r *http.Request) (result apiFuncResult) {
	...
	qry, err := api.QueryEngine.NewRangeQuery(ctx, api.Queryable, opts, r.FormValue("query"), start, end, step, false)
	if err != nil {
		return invalidParamError(err, "query")
	}
	// From now on, we must only return with a finalizer in the result (to
