...
// See https://github.com/zzylol/prometheus-sketches/issues/7846
...
func newScrapeLoop(ctx context.Context,
	...
	if passMetadataInContext {
		...
		// leak. We should ideally fix the main leak. See: https://github.com/zzylol/prometheus-sketches/pull/10590
	...
	for {
		...
		if seriesAlreadyScraped {
			...
			if isHistogram {
				...
				// PromSketch: Insert t, val into sketch instances if there is a corresponding sketch rule
				var wg_cache sync.WaitGroup
				if sl.ps != nil {
					wg_cache.Add(1)
					go func() {
						defer wg_cache.Done()
						sketch_err := sl.ps.SketchInsert(lset, t, val)
						if sketch_err != nil {
							level.Debug(sl.l).Log("msg", "PromSketch Insertion error", "series", string(met), "err", err)
						}
					}()
				}
