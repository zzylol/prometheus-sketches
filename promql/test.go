...
func (t *test) execRangeEval(cmd *evalCmd, engine QueryEngine) error {
	q, err := engine.NewRangeQuery(t.context, t.storage, nil, cmd.expr, cmd.start, cmd.end, cmd.step, false)
	if err != nil {
...
func (t *test) execInstantEval(cmd *evalCmd, engine QueryEngine) error {
	...
	for _, iq := range queries {
		q, err := engine.NewInstantQuery(t.context, t.storage, nil, iq.expr, iq.evalTime, false)
		if err != nil {
		...
		q, err = engine.NewRangeQuery(t.context, t.storage, nil, iq.expr, iq.evalTime.Add(-time.Minute), iq.evalTime.Add(time.Minute), time.Minute, false)
		if err != nil {
			return fmt.Errorf("error creating range query for %q (line %d): %w", cmd.expr, cmd.line, err)
		}
