...
type SFunctionCall func(lset labels.Labels, mint, maxt int64) (Vector, annotations.Annotations)
...
func funcDistinctOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
	if len(vals[0].(Matrix)[0].Floats) == 0 {
		// The passed values only contain
		// histograms. distinct_over_time ignores histograms for now. If
		// there are only histograms, we have to return without adding
		// anything to enh.Out.
		return enh.Out, nil
	}
...
func funcEntropyOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
	if len(vals[0].(Matrix)[0].Floats) == 0 {
		// The passed values only contain
		// histograms. entropy_over_time ignores histograms for now. If
		// there are only histograms, we have to return without adding
		// anything to enh.Out.
		return enh.Out, nil
	}
	...
	return aggrOverTime(vals, enh, func(s Series) float64 {
		...
		return math.Log2(n) - entropy/n
	}), nil
}

func funcL2OverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
	if len(vals[0].(Matrix)[0].Floats) == 0 {
		// The passed values only contain
		// histograms. l2_over_time ignores histograms for now. If
		// there are only histograms, we have to return without adding
		// anything to enh.Out.
		return enh.Out, nil
	}
...
func funcL1OverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
	if len(vals[0].(Matrix)[0].Floats) == 0 {
		// The passed values only contain
		// histograms. l1_over_time ignores histograms for now. If
		// there are only histograms, we have to return without adding
		// anything to enh.Out.
		return enh.Out, nil
	}
...
func funcDeriv(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
	...
	// https://github.com/zzylol/prometheus-sketches/issues/2674
...
var FunctionCalls = map[string]FunctionCall{
	...
	"delta":              funcDelta,
	"deriv":              funcDeriv,
	"distinct_over_time": funcDistinctOverTime,
