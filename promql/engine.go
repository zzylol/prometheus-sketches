...
import (
	...
	"github.com/zzylol/prometheus-sketches/model/timestamp"
	"github.com/zzylol/prometheus-sketches/model/value"
	...
	"github.com/zzylol/prometheus-sketches/util/stats"
	"github.com/zzylol/prometheus-sketches/util/zeropool"
)

var total_print int = 0
...
type QueryEngine interface {
	NewInstantQuery(ctx context.Context, q storage.Queryable, opts QueryOpts, qs string, ts time.Time, isrule bool) (Query, error)
	NewRangeQuery(ctx context.Context, q storage.Queryable, opts QueryOpts, qs string, start, end time.Time, interval time.Duration, isrule bool) (Query, error)
}
...
// see https://github.com/zzylol/prometheus-sketches/issues/8949.
...
func (q *query) Exec(ctx context.Context) *Result {
	...
	// fmt.Println("[promql] exec() time =", since.Seconds(), "(s)")
...
type EngineOpts struct {
	...
	SketchCache    *promsketch.PromSketches
...
func (ng *Engine) NewInstantQuery(ctx context.Context, q storage.Queryable, opts QueryOpts, qs string, ts time.Time, isrule bool) (Query, error) {
	pExpr, qry := ng.newQuery(q, qs, opts, ts, ts, 0, isrule)
...
func (ng *Engine) NewRangeQuery(ctx context.Context, q storage.Queryable, opts QueryOpts, qs string, start, end time.Time, interval time.Duration, isrule bool) (Query, error) {
	pExpr, qry := ng.newQuery(q, qs, opts, start, end, interval, isrule)
	finishQueue, err := ng.queueActive(ctx, qry)
	if err != nil {
		return nil, err
	}
...
func (ng *Engine) newQuery(q storage.Queryable, qs string, opts QueryOpts, start, end time.Time, interval time.Duration, isrule bool) (*parser.Expr, *query) {
	if opts == nil {
...
func (ng *Engine) exec(ctx context.Context, q *query) (v parser.Value, ws annotations.Annotations, err error) {
	...
	case *parser.EvalStmt:
		// t_now := time.Now()
		value, annotation, err := ng.execEvalStmt(ctx, q, s)
		// since := time.Since(t_now)
		// fmt.Println("[promql] execEvalStmt time=", since.Seconds(), "(s)")
		return value, annotation, err
...
func (ng *Engine) execEvalStmt(ctx context.Context, query *query, s *parser.EvalStmt) (parser.Value, annotations.Annotations, error) {
	...
	prepareSpanTimer.Finish()
	// since := time.Since(t_now)
	// fmt.Println("[promql] prepare time=", since.Seconds(), "(s)")
	...
	if s.Start == s.End && s.Interval == 0 {
		...
		evaluator := &evaluator{
			...
			sketchCache:              ng.sketchCache,
			isrule:                   query.isrule,
			ScrapeInterval:           ng.ScrapeInterval,
		}
		...
		case parser.ValueTypeVector:
			...
			// since = time.Since(t_now)
			// fmt.Println("[promql] eval time=", since.Seconds(), "(s)")
...
func (ev *evaluator) eval(expr parser.Expr) (parser.Value, annotations.Annotations) {
	...
	case *parser.Call:
		...
		var total_storage_duration float64 = 0
		var total_call_duration float64 = 0
		var total_sketch_query_duration float64 = 0
		var total_sketch_alloc_duration float64 = 0
		for i, s := range selVS.Series {
			...
			for ts, step := ev.startTimestamp, -1; ts <= ev.endTimestamp; ts += ev.interval {
				...
				var outVec Vector
				var annos annotations.Annotations = nil
				if ts == ev.startTimestamp || selVS.Timestamp == nil {
					maxt := ts - offset
					mint := maxt - selRange
					t_sketch := time.Now()

					// Since Prometheus subquery eval reuse the same metric name for intermediate matrix results,
					// current PromSketch cannot distinguish between a subquery and a direct query applied to a timeseries.
					// To solve this, we need to use a new metric name for evalSubquery return matrix.
					// If aggr_over_time is the outside function of a subquery, then internal subquery eval series
					// need to be inserted to PromSketch cache as well if sketches are used.

					lookup := ev.sketchCache.LookUpAndUpdateWindow(selVS.Series[i].Labels(), e.Func.Name, mint, maxt)

					/*
						// for checking data time range
						if total_print == 20 {
							fmt.Println("lookup=", lookup)
							floats, histograms = ev.matrixIterSlice(it, mint, maxt, floats, histograms)
							if len(floats) > 0 {
								fmt.Println("floats mint, maxt, len", floats[0].T, floats[len(floats)-1].T, len(floats))
							}
						}
					*/
					...
					if lookup {
						...
						/*
							// debug
							fmt.Println("sketch=", outVec)
							floats, histograms = ev.matrixIterSlice(it, mint, maxt, floats, histograms)
							inMatrix[0].Floats = floats
							inMatrix[0].Histograms = histograms
							enh.Ts = ts
							outVec, _ = call(inArgs, e.Args, enh)
							fmt.Println("acc=", outVec)
						*/

						since := time.Since(t_sketch)
						total_sketch_query_duration += since.Seconds()
						goto sketchcache
					} else {
						if ev.isrule {
							...
							since_sketch := time.Since(t_sketch)
							total_sketch_alloc_duration += since_sketch.Seconds()
							if err != nil {
						...
						t_storage := time.Now()
						floats, histograms = ev.matrixIterSlice(it, mint, maxt, floats, histograms)
						since_storage := time.Since(t_storage)
						total_storage_duration += since_storage.Seconds()
					}
				}
...

				{
					...
					since := time.Since(t_now)
					total_call_duration += since.Seconds()
		...
		if total_print == 20 {
			fmt.Println("[promql] sketch alloc time=", total_sketch_alloc_duration, "(s)")
			fmt.Println("[promql] sketch query time=", total_sketch_query_duration, "(s)")
			fmt.Println("[promql] read from storage time=", total_storage_duration, "(s)")
			fmt.Println("[promql] call time=", total_call_duration, "(s)")
			total_print = 0
		}
		total_print += 1
...
func (ev *evaluator) rangeEvalTimestampFunctionOverVectorSelector(vs *parser.VectorSelector, call FunctionCall, e *parser.Call) (parser.Value, annotations.Annotations) {
	...
	return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (Vector, annotations.Annotations) {
		if vs.Timestamp != nil {
			...
			// See https://github.com/zzylol/prometheus-sketches/issues/8433.
...
func copy_prom_vec(vec promsketch.Vector) Vector {
	out := make(Vector, 0)
	for i := 0; i < len(vec); i++ {
		out = append(out, Sample{F: vec[i].F, T: vec[i].T})
	}
	return out
}
