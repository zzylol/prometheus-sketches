...
import (
	...
	"github.com/zzylol/prometheus-sketches/util/teststorage"
...
func TestQueryError(t *testing.T) {
	...
	vectorQuery, err := engine.NewInstantQuery(ctx, queryable, nil, "foo", time.Unix(1, 0), false)
	require.NoError(t, err)
	...
	matrixQuery, err := engine.NewInstantQuery(ctx, queryable, nil, "foo[1m]", time.Unix(1, 0), false)
	require.NoError(t, err)
...
func TestSelectHintsSetCorrectly(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.query, func(t *testing.T) {
			...
			if tc.end == 0 {
				query, err = engine.NewInstantQuery(ctx, hintsRecorder, nil, tc.query, timestamp.Time(tc.start), false)
			} else {
				query, err = engine.NewRangeQuery(ctx, hintsRecorder, nil, tc.query, timestamp.Time(tc.start), timestamp.Time(tc.end), time.Second, false)
			}
			require.NoError(t, err)
...
`)
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("%d query=%s", i, c.Query), func(t *testing.T) {
			...
			if c.Interval == 0 {
				qry, err = engine.NewInstantQuery(context.Background(), storage, nil, c.Query, c.Start, false)
			} else {
				qry, err = engine.NewRangeQuery(context.Background(), storage, nil, c.Query, c.Start, c.End, c.Interval, false)
			}
			require.NoError(t, err)
	...
	for _, c := range cases {
		t.Run(c.Query, func(t *testing.T) {
			...
			runQuery := func(expErr error) *stats.Statistics {
				...
				var qry Query
				if c.Interval == 0 {
					qry, err = engine.NewInstantQuery(context.Background(), storage, opts, c.Query, c.Start, false)
				} else {
					qry, err = engine.NewRangeQuery(context.Background(), storage, opts, c.Query, c.Start, c.End, c.Interval, false)
				}
				require.NoError(t, err)
...
`)
	...
	// To test the fix for https://github.com/zzylol/prometheus-sketches/issues/8433.
	...
	cases := []struct {
		...
		{ // Time of the result is the evaluation time.
			...
			// Tests for https://github.com/zzylol/prometheus-sketches/issues/8433.
	...
	for _, c := range cases {
		t.Run(c.query, func(t *testing.T) {
			...
			var qry Query
			if c.end == 0 {
				qry, err = engine.NewInstantQuery(context.Background(), storage, nil, c.query, start, false)
			} else {
				qry, err = engine.NewRangeQuery(context.Background(), storage, nil, c.query, start, end, interval, false)
			}
			require.NoError(t, err)
...
`)
	...
	qry, err := engine.NewRangeQuery(context.Background(), storage, nil, query, start, end, interval, false)
	require.NoError(t, err)
...
func TestEngineOptsValidation(t *testing.T) {
	...
	for _, c := range cases {
		...
		_, err1 := eng.NewInstantQuery(context.Background(), nil, nil, c.query, time.Unix(10, 0), false)
		_, err2 := eng.NewRangeQuery(context.Background(), nil, nil, c.query, time.Unix(0, 0), time.Unix(10, 0), time.Second, false)
		if c.fail {
...
func TestNativeHistogramRate(t *testing.T) {
	...
	t.Run("instant_query", func(t *testing.T) {
		qry, err := engine.NewInstantQuery(context.Background(), storage, nil, queryString, timestamp.Time(int64(5*time.Minute/time.Millisecond)), false)
		require.NoError(t, err)
		res := qry.Exec(context.Background())
		require.NoError(t, res.Err)
	...
	t.Run("range_query", func(t *testing.T) {
		...
		qry, err := engine.NewRangeQuery(context.Background(), storage, nil, queryString, start, end, step, false)
		require.NoError(t, err)
		res := qry.Exec(context.Background())
		require.NoError(t, res.Err)
...
func TestNativeHistogram_HistogramCountAndSum(t *testing.T) {
	...
	for _, floatHisto := range []bool{true, false} {
		t.Run(fmt.Sprintf("floatHistogram=%t", floatHisto), func(t *testing.T) {
			...
			qry, err := engine.NewInstantQuery(context.Background(), storage, nil, queryString, timestamp.Time(ts), false)
			require.NoError(t, err)
...
`
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			qry, err := engine.NewInstantQuery(context.Background(), storage, opts, query, c.ts, false)
			require.NoError(t, err)
