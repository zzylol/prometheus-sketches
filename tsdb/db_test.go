...
import (
	...
	"github.com/zzylol/prometheus-sketches/model/metadata"
	"github.com/zzylol/prometheus-sketches/storage"
	"github.com/zzylol/prometheus-sketches/tsdb/chunkenc"
	"github.com/zzylol/prometheus-sketches/tsdb/chunks"
	"github.com/zzylol/prometheus-sketches/tsdb/fileutil"
	"github.com/zzylol/prometheus-sketches/tsdb/index"
	"github.com/zzylol/prometheus-sketches/tsdb/record"
	"github.com/zzylol/prometheus-sketches/tsdb/tombstones"
	"github.com/zzylol/prometheus-sketches/tsdb/tsdbutil"
	"github.com/zzylol/prometheus-sketches/tsdb/wlog"
	"github.com/zzylol/prometheus-sketches/util/annotations"
	"github.com/zzylol/prometheus-sketches/util/testutil"
)
...
func TestMain(m *testing.M) {
	...
	goleak.VerifyTestMain(m, goleak.IgnoreTopFunction("github.com/zzylol/prometheus-sketches/tsdb.(*SegmentWAL).cut.func1"), goleak.IgnoreTopFunction("github.com/zzylol/prometheus-sketches/tsdb.(*SegmentWAL).cut.func2"))
}
...
// https://github.com/zzylol/prometheus-sketches/issues/7548
...
// See https://github.com/zzylol/prometheus-sketches/issues/5105
...
// https://github.com/zzylol/prometheus-sketches/issues/9846
// https://github.com/zzylol/prometheus-sketches/issues/9859
...
// Regression test for https://github.com/zzylol/prometheus-sketches/pull/6514.
...
// TestCompactHeadWithDeletion tests https://github.com/zzylol/prometheus-sketches/issues/11585.
...
// Tests https://github.com/zzylol/prometheus-sketches/issues/10291#issuecomment-1044373110.
...
// Regression test for https://github.com/zzylol/prometheus-sketches/pull/13754
...
func ingestScrapes(st *DB, lbls []labels.Labels, scrapeCount int, promcache *promsketch.PromSketches) (uint64, error) {
	...
	for i := 0; i < scrapeCount; i += 100 {
		...
		for len(lbls) > 0 {
			...
			go func() {
				...
				n, err := ingestScrapesShard(st, batch, 100, int64(timeDelta*i), promcache)
				if err != nil {
					// exitWithError(err)
					fmt.Println(" err", err)
				}
...
func ingestScrapesZipf(st *DB, lbls []labels.Labels, scrapeCount int, promcache *promsketch.PromSketches) (uint64, error) {
	var total atomic.Uint64
	ts_per_worker := len(lbls)/64 + 1
	...
	for i := 0; i < scrapeCount; i += 100 {
		...
		for len(lbls) > 0 {
			...
			go func() {
				...
				n, err := ingestScrapesShardZipf(st, batch, 100, int64(timeDelta*i), z, promcache)
				if err != nil {
					// exitWithError(err)
					fmt.Println(" err", err)
				}
...
func ingestScrapesShard(st *DB, lbls []labels.Labels, scrapeCount int, baset int64, promcache *promsketch.PromSketches) (uint64, error) {
	ts := baset
	...
	scrape := make([]*samplebench, 0, len(lbls))

	for i, m := range lbls {
		scrape = append(scrape, &samplebench{
			labels: m,
			value:  make([]int64, 0),
		})
		for j := 0; j < scrapeCount; j++ {
			scrape[i].value = append(scrape[i].value, int64(rand.NormFloat64()*100000))
		}
	}
	total := uint64(0)
			...
			for j := 0; j < len(scrape); j += 10 {
				...
				go func(j int) {
					for k := j; k < j+10; k++ {
						...
						err := promcache.SketchInsert(scrape[k].labels, ts, float64(scrape[k].value[i]))
						if err != nil {
							panic(err)
						}
					}
	...
	for i := 0; i < scrapeCount; i++ {
		...
		for _, s := range scrape {
			...
			ref, err := app.Append(ref, s.labels, ts, float64(s.value[i]))
			if err != nil {
				panic(err)
			}
...
func ingestScrapesShardZipf(st *DB, lbls []labels.Labels, scrapeCount int, baset int64, z *rand.Zipf, promcache *promsketch.PromSketches) (uint64, error) {
	ts := baset
	...
	for i, m := range lbls {
		...
		for j := 0; j < scrapeCount; j++ {
			scrape[i].value = append(scrape[i].value, int64(z.Uint64()))
			// scrape[i].value = append(scrape[i].value, int64(rand.Float64()*100000))
		}
	}
	...
	go func() {
		...
		for i := 0; i < scrapeCount; i++ {
			...
			for j := 0; j < len(scrape); j += 1 {

				if j >= len(scrape) {
					break
				}
				err := promcache.SketchInsert(scrape[j].labels, ts, float64(scrape[j].value[i]))
				if err != nil {
					panic(err)
				}
				ato_total.Add(1)
	...
	for i := 0; i < scrapeCount; i++ {
		...
		for j, s := range scrape {
			...
			ref, err := app.Append(ref, s.labels, ts, float64(scrape[j].value[i]))
			if err != nil {
				panic(err)
			}
	...
	total = ato_total.Load()
	return total, nil
}

func TestDBInsertionThroughputNormal(t *testing.T) {
	st, err := Open(t.TempDir(), nil, nil, &Options{
	...
	var total_duration atomic.Float64
	total_duration.Store(0)
	...
	for j := 0; j < num_ts; j++ {
		...
		promcache.NewSketchCacheInstance(inputLabel, "quantile_over_time", 10000000, 100000, 10000)
		promcache.NewSketchCacheInstance(inputLabel, "sum_over_time", 10000000, 100000, 10000)
		promcache.NewSketchCacheInstance(inputLabel, "count_over_time", 10000000, 100000, 10000)
		// promcache.NewSketchCacheInstance(inputLabel, "entropy_over_time", 100000000, 100000, 10000)
	}

	start := time.Now()
	ingestScrapes(st, lsets, scrapeCountBatch, promcache)
...
func cleanFiles() {
	files, err := filepath.Glob("/mydata/go-build*")
	if err != nil {
		panic(err)
	}
	for _, f := range files {
	...
	files, err = filepath.Glob("/mydata/TestDBInsertion*")
	if err != nil {
		panic(err)
	}
	for _, f := range files {
	...
	os.RemoveAll("/users/zz_y/.cache")
}

func TestDBInsertionThroughputZipf(t *testing.T) {
	cleanFiles()
	...
	for j := 0; j < num_ts; j++ {
		...
		promcache.NewSketchCacheInstance(inputLabel, "quantile_over_time", 1000000, 10000, 10000)
		// promcache.NewSketchCacheInstance(inputLabel, "entropy_over_time", 100000000, 1000000, 10000)
	}

	start := time.Now()
	ingestScrapesZipf(st, lsets, scrapeCountBatch, promcache)
